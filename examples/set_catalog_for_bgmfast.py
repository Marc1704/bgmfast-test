...
sel_columns_catalog = ['phot_g_mean_mag', 'bp_rp', 'g_rp', 'l', 'b', 'parallax']
...
desired_columns = ['G', 'BpRp', 'GRp', 'longitude', 'latitude']
catalog_file.change_column_name(old_columns_names, desired_columns)

#Apply the basic data filter
print(catalog_file.df['all'])
catalog_file.basic_filter(colnames={'Parallax': 'parallax'})

#Compute absolute magnitudes
catalog_file.compute_absolute_magnitude(colnames={'G': 'G', 'Parallax': 'parallax'})

#Save the final catalog with absolute magnitudes and the proper columns names
final_columns = desired_columns.append('Mvarpi')
catalog_file.save(catalog_out_name, columns=final_columns)
